package com.guotg.depository.common.rocketmq;
...
public class FileConsumer {
     ...
     * 生产者的组名
     */
    @Value("${depository.rocketmq.consumerGroup}")
            ...
            consumer.registerMessageListener((MessageListenerConcurrently) (list, context) -> {//->为Java8的lambda表达式,就是匿名函数,具体可以参考该文章https://segmentfault.com/q/1010000007518474。
                try {
                    for (MessageExt messageExt : list) {
                        ...
                        imagService.compress(messageBody);

                        Logger.log("消费响应：msgId : " + messageExt.getMsgId() + ",  msgBody : " + messageBody);//输出消息内容
                    }
                } catch (Exception e) {
                    Logger.error(e);
